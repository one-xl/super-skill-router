...
| **Author / edit an HTML composition** — the `data-*` contract, clips, tracks, sub-compositions, variables                                        | `/core`                          |
| **Animate** — atomic motion rules, scene blueprints, transitions, runtime adapters (GSAP / Lottie / Three.js / Anime.js / CSS / WAAPI / TypeGPU) | `/animation`                     |
| **Creative direction** — `design.md`, palettes, typography, narration, beat planning, audio-reactive                                             | `/creative`                      |
| **Media preprocessing** — TTS voiceover, background music, transcription, background removal, captions                                           | `/media`                         |
| **CLI dev loop** — init, lint, validate, inspect, preview, render, publish, doctor                                                               | `/cli`                           |
| **Install registry blocks / components** (`hyperframes add`)                                                                                     | `/registry`                      |

> The composition **authoring contract** (every timed element needs `data-start` / `data-duration` / `data-track-index`; timed elements need `class="clip"`; GSAP timelines are paused and registered on `window.__timelines`; deterministic logic only — no `Date.now()` / `Math.random()` / network) is **not duplicated here** — it lives in `/core`. Read that before writing composition HTML.
...
This section knows ONLY top-level workflows. It does not load workflow-internal phases, domain skills (`*` — see the capability map above), or technical references.
...
- **Output:** A HyperFrames HTML composition (any length / format) authored with the original `hyperframes` flow: design system → prompt expansion → plan → layout-before-animation → build (delegating to the `*` domain skills) → validate.
...
- **Domain skills** (`/core`, `/animation`, `/cli`, `/creative`, `/media`, `/registry`) — these are NOT routed here, but they ARE in the **capability map** at the top of this skill; a workflow's build phase loads them as technical references.
