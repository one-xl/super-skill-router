...
> Read this file **only when** authoring a full scene from a pre-designed multi-phase choreography template. For atomic motion techniques, the rules index in [SKILL.md](./INSTRUCTION.md) is the default starting point.
...
If no blueprint fits, **compose rules directly** from [SKILL.md](./INSTRUCTION.md) — blueprints are starting points, not requirements.
