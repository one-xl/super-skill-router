...
> **Harness note:** "Skill `X`" = load skill X via your harness's skill mechanism; without one, read `<SKILL_DIR>/../X/INSTRUCTION.md` directly. `Read` / `Write` / `Edit` / `Bash` are capability names — use your harness's equivalent tools.
