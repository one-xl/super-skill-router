...
At build time, the sub-agent loads `/animate-text` (the upstream skill) and reads each named effect's spec from `.agents/internal-skills/animate-text/assets/effects/<id>.json`. No creative decisions at build time — just spec retrieval and implementation.
