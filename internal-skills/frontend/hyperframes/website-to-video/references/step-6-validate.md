...
[ ] animation-map.json generated          → run `node <repo-root>/internal-skills/hyperframes/scripts/animation-map.mjs <project-dir>`; confirm every beat has events listed and no bbox/flag warnings
[ ] w2h-verify report                     → run `node <repo-root>/internal-skills/website-to-video/scripts/w2h-verify.mjs <project-dir>`; paste the FULL output (every row, every percent) verbatim into your final user-facing summary — see "w2h-verify — the source of truth" below
...
```bash
node <repo-root>/internal-skills/website-to-video/scripts/w2h-verify.mjs <project-dir>
```

(Locate the repo root from a project subdirectory: `find "$HOME" -path '*/internal-skills/website-to-video/scripts/w2h-verify.mjs' -maxdepth 10 2>/dev/null | head -1`.)
