# AI-Conference Pipeline

Use this reference when mode is `ai-conference` or when `hybrid` mode needs research execution, experiments, or conference-style evidence gates.
...
4. `LITERATURE_COLLECT`: Use real sources such as OpenAlex, Semantic Scholar, arXiv, DOI/Crossref, PubMed when relevant, official proceedings, and publisher pages. Collect at least 30 genuinely relevant papers when possible.
...
13. `ITERATIVE_REFINE`: Repair runtime bugs, NaN/Inf, numerical instability, and weak experimental coverage by diagnosing root causes.
...
22. `EXPORT_PUBLISH`: Export to LaTeX using the target conference template.
...
Gate stages are 5, 9, and 20. If `--auto-approve` is absent, present the gate artifact, recommendation, risk summary, and exact next step before proceeding.
...
- figures generated from actual result files

Paper:

- `paper/mypaper/main.tex`
- section files under `paper/mypaper/sections/`
- figures under `paper/mypaper/figures/`

## External Tools

Use configured environment variables rather than hard-coded credentials:
...
Use external coding or research tools only when actually installed/configured and allowed by the sandbox. If unavailable, document the fallback.
